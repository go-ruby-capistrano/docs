...
`go-ruby-capistrano/capistrano` reimplements Ruby's `capistrano` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-capistrano/capistrano` directly, with no Ruby runtime;
...
Because `capistrano` is pure Go with cgo disabled, it also compiles to **WebAssembly**
